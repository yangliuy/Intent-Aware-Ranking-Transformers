...
def dot_sim_2d(x, y):
    '''
    calculate dot similarity with two tensor in 2D for intent attention
    '''
    M = tf.get_variable(
        name="dot_attention_matrix",
        shape=[x.shape[-1]*2], # concate x and y
        dtype=tf.float32,
        initializer=tf.truncated_normal_initializer(stddev=0.02))
    con = tf.concat([x,y], 1)
    sim = tf.einsum('bi,i->b', con, M)
    return sim

def bilinear_sim_2d(x, y):
    '''
    calculate bilinear similarity with two tensor in 2D for intent attention
    '''
    M = tf.get_variable(
        name="bilinear_matrix",
        ...
        dtype=tf.float32,
        initializer=tf.truncated_normal_initializer(stddev=0.02))
    sim = tf.einsum('bi,ij,jb->b', x, M, tf.transpose(y))
    return sim

def outprod_sim_2d(x, y):
    '''
    calculate outprod similarity with two tensor in 2D for intent attention
    '''
    M = tf.get_variable(
        name="outproduct_matrix",
        shape=[x.shape[-1]*y.shape[-1]],
        dtype=tf.float32,
        initializer=tf.truncated_normal_initializer(stddev=0.02))
    sim = tf.einsum('bi,bj->bij', x, y)
    flat = tf.reshape(sim, [x.shape[0],-1]) # [batch, intent_size*intent_size]
    sim = tf.einsum('bi,i->b', flat, M)
    return sim
...
def mask(row_lengths, col_lengths, max_row_length, max_col_length):
    ...
    Args:
        ...
        row_lengths and col_lengths are real lengths
        max_row_length and max_col_length are max lengths
    ...
    Raises:
    '''
    row_mask = tf.sequence_mask(row_lengths, max_row_length) #return bool, [batch, max_row_len]
    col_mask = tf.sequence_mask(col_lengths, max_col_length) #return bool, [batch, max_col_len]
